...
function listsListing(){
  const listingContainerElementId = document.getElementById
  (listingContainerElementId);

  if (!listingContainerElementId){
    return;
  }
  ...
  .then((listing) => { 
    if(!listing){               
      listingContainerElementId.innerHTML = 'No lisitngs fetched';
      return;
    }

    for(const listing of listing ){
      listingContainerElementId.appendChild(listingElement(listing));
    }
  })
  .catch((e) => {
    console.log(e);
